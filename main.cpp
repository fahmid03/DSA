...
public:
    Graph(int v,int e)
    {
        ...
        adj.resize(V+1);//resize na korle purai mara
        for (int i=0;i<E;i++)
        {
    ...
    int djikstra(int src)
    {
        ...
        while (!pq.empty())
        {
            ...
            int u=top[1];
            int coupon=top[2];
            if (coupon == 0 && d != nocoupondist[u]) continue;
            if (coupon == 1 && d != coupondist[u]) continue;
            for (auto edge : adj[u])
            {
                int v = edge.first;
                int w = edge.second;
                ...
                {
                    coupondist[v] = coupondist[u] + w/2;
                    pq.push({coupondist[v], v,1});
                }
            }
        }
        return min(nocoupondist[V],coupondist[V]);
    }
