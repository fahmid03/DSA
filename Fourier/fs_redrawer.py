...
class FourierEpicycles:
    ...
    def calculate_cn(self, n):
        ...
        """
        # TODO: implement this method
        hardterm=np.exp(-1j*self.omega*n*self.t)
        hardterm=self.signal*hardterm
        easyterm=self.t
        c_n=np.trapezoid(hardterm, easyterm)/self.T
        return c_n
    def calculate_all_coefficients(self):
        ...
        for i in range(-self.N,self.N+1):
            self.coeffs[i]=self.calculate_cn(i)
    def approximate(self, t):
        ...
        """
        # TODO: implement this method
        newdrawing=np.zeros_like(t,dtype=complex)
        for i,c_i in self.coeffs.items():
            currentcircle=c_i*np.exp(1j*i*self.omega*t)
            newdrawing+=currentcircle
        return newdrawing
...
if __name__ == "__main__":
    ...
    t, z = load_svg_path(svg_path, num_points=5000)
