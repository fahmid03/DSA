...
class CFT2D:
    ...
    def compute_cft(self):
        ...
        height,width=self.I.shape
        freqheight=len(self.v)
        freqwidth=len(self.u)
        cosx=np.zeros((freqwidth,height))
        sinx=np.zeros((freqwidth,height))
        for uindex in range(freqwidth):
            uval=self.u[uindex]
            for yindex in range(height):
                imageslice=self.I[yindex,:]
                cosslice=np.cos(2*np.pi*uval*self.x)
                sinslice=np.sin(2*np.pi*uval*self.x)
                cosx[uindex,yindex]=np.trapezoid(imageslice*cosslice,self.x)
                sinx[uindex,yindex]=np.trapezoid(imageslice*sinslice,self.x)
        real=np.zeros((freqheight,freqwidth))
        imag=np.zeros((freqheight,freqwidth))
        for vindex in range(freqheight):
            vval=self.v[vindex]
            for uindex in range(freqwidth):
                cosxcolumn=cosx[uindex,:]
                sinxcolumn=sinx[uindex,:]
                cos=np.cos(2*np.pi*vval*self.y)
                sin=np.sin(2*np.pi*vval*self.y)
                realcos=np.trapezoid(cosxcolumn*cos,self.y)
                realsin=np.trapezoid(sinxcolumn*sin,self.y)
                imagcos=np.trapezoid(sinxcolumn*cos,self.y)
                imagsin=np.trapezoid(cosxcolumn*sin,self.y)
                real[vindex,uindex]=realcos-realsin
                imag[vindex,uindex]=(imagcos+imagsin)
        return real, imag
    ...
    def plot_magnitude(self):
        ...
        logval=np.log(1+modulus)
        plt.imshow(logval, cmap='gray',extent=[self.u[0],self.u[-1],self.v[0],self.v[-1]])
        plt.xlabel("u")
        plt.ylabel("v")
        plt.show()
...
class InverseCFT2D:
    ...
    def reconstruct(self):
        ...
        """
        # TODO: implement this method
        freqheight,freqwidth=self.real.shape
        width=len(self.x)
        height=len(self.y)
        firstreal=np.zeros((freqwidth,height))
        firstimag=np.zeros((freqwidth,height))
        for uindex in range(freqwidth):
            realcol=self.real[:,uindex]
            imagcol=self.imag[:,uindex]
            for yindex in range(height):
                yval=self.y[yindex]
                wavecos=np.cos(2*np.pi*yval*self.v)
                wavesin=np.sin(2*np.pi*yval*self.v)
                term1=realcol*wavecos-imagcol*wavesin
                term2=realcol*wavesin+imagcol*wavecos
                firstreal[uindex,yindex]=np.trapezoid(term1,self.v)
                firstimag[uindex,yindex]=np.trapezoid(term2,self.v)
        reconstructed=np.zeros((height,width))
        for yindex in range(height):
            real=firstreal[:,yindex]
            imag=firstimag[:,yindex]
            for xindex in range(width):
                xval=self.x[xindex]
                wavecos=np.cos(2*np.pi*xval*self.u)
                wavesin=np.sin(2*np.pi*xval*self.u)
                term=real*wavecos+imag*wavesin
                reconstructed[yindex,xindex]=np.trapezoid(term,self.u)
        return np.flipud(reconstructed)
# =====================================================
